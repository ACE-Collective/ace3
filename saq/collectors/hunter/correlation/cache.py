...
def _describe_command(command_args: dict) -> str:
    ...
    Deliberately omits `env` for executables: env is where a hunt puts the settings it hands a
    script, and this description is written to the log. The rendered env still participates in
    the cache key, but only as part of a sha256 digest.
    """
