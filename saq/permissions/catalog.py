...
PERMISSION_CATALOG: tuple[CatalogEntry, ...] = (
    ...
    # its version token, saq.log, package download), ai:event (ACE Event read) and ai:search (alert
    # search) are the only static entries; the per-backend ai:<name> entries are DERIVED from the deployment's
    ...
    CatalogEntry("ai", "search", "Search alerts (exact indicator/tag matches plus semantic matches) and find similar alerts via the AI investigation API."),
