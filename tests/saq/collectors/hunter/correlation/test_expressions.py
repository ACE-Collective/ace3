...
class TestBuildJinjaContext:
    ...
    def test_context_is_event_data_only(self):
        """A hunt template has no path to configuration or credentials."""
        ctx = build_jinja_context({}, [])
        assert set(ctx) == {"_event", "_events"}

    def test_context_takes_no_config_or_secrets(self):
        with pytest.raises(TypeError):
            build_jinja_context({}, [], {"global": {"key": "value"}})
        with pytest.raises(TypeError):
...
class TestEvaluateExpression:
    ...
    def test_jinja_cannot_access_config(self):
        expr = ExpressionConfig(type="jinja", value="{{ _config is undefined }}")
        assert evaluate_expression(expr, {}, []) is True
