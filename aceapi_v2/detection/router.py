...
from aceapi_v2.detection.schemas import DetectionUpdate, ExpirationUpdate, ObservableDetectionRead

router = APIRouter(dependencies=[Security(get_current_auth)])


@router.get("/", response_model=list[ObservableDetectionRead])
async def list_observables(
    ...
    return await service.list_detection_observables(
        session, for_detection=for_detection, search=search, limit=limit, offset=offset
    )
