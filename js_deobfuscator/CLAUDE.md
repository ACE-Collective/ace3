...
**Blob side-channel.** Text-MIME `new Blob([...])` payloads are written as sibling files so ACE's extraction pipeline can recurse into them (`html_js_extraction` → another deob pass). The extension is load-bearing; ACE names them `<source>.blob_<n>.<ext>` so the extension gates downstream still match. Bounded at `MAX_BLOB_FILES`/`MAX_BLOB_BYTES`. Add new side effects via the `constructHooks`/`callHooks` tables rather than editing `recorder()`. `document.write`/`writeln` use the same channel: every string fragment the sample writes is accumulated and flushed after the run as one `blob_<n>.html` (a browser builds one document from all the writes, and a `<script>` split across two writes never parses as two files). Without it a sample that writes a whole second document survives only as a string literal in the trace and the nested `<script>` never runs.

**DOM snapshot.** For scripts extracted from HTML/SVG, `html_js_extraction` ships a `<script>.dom.json` sidecar the harness reads at `INPUT_PATH + '.dom.json'`. It backs `document.getElementById`/`querySelector`/`getElementsBy*` and element attribute reads (`getAttribute`, `dataset`, `textContent`, `value`, and `content` — a real fragment on `<template>`, the reflected attribute on `<meta>`, `undefined` elsewhere, because kits read pieces through `el.content ? el.content.textContent : el.textContent` and a truthy recorder takes the wrong branch) with the source document's **real** values via the recorder's optional `overrides` map, and `document.documentElement`/`body`/`head` resolve to the wrapper for the first `html`/`body`/`head` element (kits stash the pieces as custom attributes on the `<html>` tag). Attribute lookups are exact-match first and then lowercase: the HTML parser has already lowercased every stored name, while the script asks for the mixed-case name it authored — the canonical case is a phish whose redirect URL is not in the script at all but split across a sibling element's `data-*` attributes, which without real values decodes to a `[label]` placeholder and is lost silently (the recorder-loss failure mode again). A miss returns a *recorder*, not `null`, so a later `.getAttribute` can't TypeError mid-trace and lose everything after it; the `=== null` branch taking the wrong path is the accepted trade. Element-registered listeners (`addEventListener`, and `on*` property assignment via the recorder's `onSet` hook) are queued into `elementListeners` and fired **after** the DOM-ready drain and again after the async-continuation drain — the dominant shape registers the click handler from inside a `DOMContentLoaded` handler. The `querySelector` subset is deliberately `#id`/`.class`/bare-tag only; anything else degrades to a recorder. Without a sidecar the harness is byte-identical to before except for the `dom_snapshot: false` report field.
